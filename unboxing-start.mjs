...
const staticBlock="app.use(express.json({limit:'8mb'}));app.use(express.static(path.join(dir,'public')));";
const noCacheBlock="app.use(express.json({limit:'8mb'}));app.use((r,s,n)=>{s.setHeader('Cache-Control','no-store, no-cache, must-revalidate, proxy-revalidate');s.setHeader('Pragma','no-cache');s.setHeader('Expires','0');n()});app.use(express.static(path.join(dir,'public'),{etag:false,lastModified:false,maxAge:0}));";
source=mustReplace(source,staticBlock,noCacheBlock,'Cache da interface');

const oldHistoryApi="app.get('/api/history',auth,async(_,s)=>s.json((await read()).creatives.slice(0,150)));";
const newHistoryApi=`${oldHistoryApi}app.delete('/api/history',auth,async(r,s)=>{try{const ids=[...new Set((Array.isArray(r.body?.ids)?r.body.ids:[]).map(String).filter(Boolean))];if(!ids.length)return s.status(400).json({error:'Selecione pelo menos um item do histórico.'});const st=await read(),before=st.creatives.length,idset=new Set(ids);st.creatives=st.creatives.filter(x=>!idset.has(String(x.id)));const deleted=before-st.creatives.length;await write(st);s.json({ok:true,deleted,remaining:st.creatives.length})}catch(e){console.error('history-delete',e);s.status(500).json({error:e.message})}});`;
...
const takesBlock=durationBlock+'<div><label>Quantidade de takes</label><select id="takesCount"><option value="1">1 take — hook + corpo + CTA</option><option value="2" selected>2 takes — mais dinâmico</option></select></div>';
...
const newHistoryFunction=`let selectedHistory=new Set(),historyDeleteArmedUntil=0;function updateHistorySelection(){const n=selectedHistory.size,$count=$('#historySelectedCount'),$del=$('#historyDeleteSelected'),$all=$('#historySelectAll');if($count)$count.textContent=n+' selecionado'+(n===1?'':'s');if($del){$del.disabled=n===0;if(Date.now()>historyDeleteArmedUntil)$del.textContent='Apagar selecionados'}if($all)$all.textContent=history.length&&n===history.length?'Desmarcar todos':'Selecionar todos'}async function loadHistory(){history=await api('/api/history');selectedHistory=new Set();historyDeleteArmedUntil=0;$('#historyList').innerHTML=history.length?history.map(x=>\`<div class="card"><label style="display:flex;align-items:center;gap:10px;text-transform:none;font-size:14px;cursor:pointer"><input type="checkbox" data-history-id="\${esc(x.id)}" style="width:20px;height:20px;accent-color:#ff6900"><span><b>\${esc(x.productName)}</b><br><span class="status">\${esc(x.creative?.hook_escolhido||'Sem hook registrado')}</span></span></label></div>\`).join(''):'<div class="status">O histórico está vazio.</div>';document.querySelectorAll('[data-history-id]').forEach(c=>c.onchange=()=>{c.checked?selectedHistory.add(c.dataset.historyId):selectedHistory.delete(c.dataset.historyId);historyDeleteArmedUntil=0;updateHistorySelection()});updateHistorySelection()}$('#historySelectAll').onclick=()=>{if(selectedHistory.size===history.length)selectedHistory.clear();else selectedHistory=new Set(history.map(x=>String(x.id)));document.querySelectorAll('[data-history-id]').forEach(c=>c.checked=selectedHistory.has(c.dataset.historyId));historyDeleteArmedUntil=0;updateHistorySelection()};$('#historyDeleteSelected').onclick=async()=>{if(!selectedHistory.size)return;const b=$('#historyDeleteSelected'),total=selectedHistory.size;if(Date.now()>historyDeleteArmedUntil){historyDeleteArmedUntil=Date.now()+5000;b.textContent='Confirmar exclusão de '+total;return}b.disabled=true;b.textContent='Apagando...';try{const d=await api('/api/history',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({ids:[...selectedHistory]})});await loadHistory();$('#historySelectedCount').textContent=(d.deleted||0)+' apagado'+((d.deleted||0)===1?'':'s')}catch(err){alert(err.message);b.disabled=false;b.textContent='Tentar novamente';historyDeleteArmedUntil=0}};`;
front=mustReplace(front,oldHistoryFunction,newHistoryFunction,'Função de histórico');

front=front.replace('</head>','<meta http-equiv="Cache-Control" content="no-store, no-cache, must-revalidate"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="0"></head>');
front=front.replace("'\"':'&quot'","'\"':'&quot;'");
